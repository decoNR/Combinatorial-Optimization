...
\begin{solution}
  ...
  Given $ h_1 \in H_{1}^{\textnormal{min}}$, I will prove that $ h_1 \in H_{2}$ and then that $ h_1 \in H_{2}^{\textnormal{min}}$. $h_1$ is the set of all the outwards arcs from a set of vertices $X$, that includes $r$ and excludes $s$. Given that it is impossible to have an $rs$-path if there are no arcs from a set that includes $r$ to the other vertices of $V(D)$ that includes $s$, $h_1 \subseteq H_{2}$. 
